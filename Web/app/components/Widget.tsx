  ...
  function SortableItem({
    ...
    return (
      <div
        ref={setNodeRef}
        style={style}
        className={`mb-16 ${isDragging ? "opacity-50" : "opacity-100"}`}
      >
        <div className="flex items-center gap-4 mb-6 group">
          ...
          <div
            {...attributes}
            {...listeners}
            className="cursor-grab p-2 text-slate-300 hover:text-[#004fb6] transition-colors"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {widgets.map((widget: any) =>
            design === "style1" ? (
              <div
                key={widget.id}
                className="group bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm hover:shadow-xl hover:border-[#004fb6] transition-all duration-300"
              >
                ...
                <div className="p-6">
                  <div className="text-[11px] font-bold text-[#004fb6] uppercase tracking-widest mb-2">
                    ID: {widget.id}
                  </div>
        ...
        <div className="pb-20">
          {categoryOrder.map((cat) => (
            <SortableItem
              key={cat}
              category={cat}
              widgets={data[cat]?.widgets || []}
              design={data[cat]?.design}
            />
          ))}
        </div>
      </SortableContext>
